...
class RootFilePathReader {
  static Future<String> getRootFilePath(Archive epubArchive) async {
    ...
    xml.XmlDocument containerDocument = xml.parse(Utf8Codec().decode(containerFileEntry.content));
    ...
    xml.XmlElement rootFileElement = packageElement.descendants.firstWhere(
        (xml.XmlNode testElem) => (testElem is xml.XmlElement) && "rootfile" == testElem.name.local,
        orElse: () => null);
